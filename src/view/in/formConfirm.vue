    ...
    <Card :padding="5" dis-hover ref="card">
        <div style="text-align:center">
            ...
            <Row :gutter="16" style="line-height: 32px;padding:5px">
                ...
                <Col span="3">
                <DatePicker style="width: 100%;" disabled format='yyyy-MM-dd' v-model='startDate' type="date" confirm placeholder="选择开始日期"></DatePicker>
                </Col>
                <Col span="1">至</Col>
                <Col span="3">
                <DatePicker style="width: 100%;" disabled format='yyyy-MM-dd' v-model='endDate' type="date" confirm placeholder="选择结束日期"></DatePicker>
                </Col>
            </Row>
            <Row :gutter="16" style="line-height: 32px;padding:5px">
                <Col span="2"> 存货大类
                ...
                <Col span="1" :offset="1">
                <Button type="info" @click="handleNotity" :disabled="isAuditDone">通知供应商</Button>
                </Col>
            </Row>
...
import { queryInvPrice, confirmForm, loadAsk, notifyConfirm } from '@/api/ask.js'
            ...
            loadAsk({ id: this.$route.query.id }).then(({ data: { code, data, message } }) => {
                if (code == 200) {
                    this.tableData.forEach((row, index) => {
                        ...
                        const _confirmPrice = t.map(m => m.priceCurrentConfirm).sort()[0];
                        const _currentPrice = t.map(m => m.priceCurrent).sort()[0]
                        row['p_confirm_price'] = _confirmPrice == 0 ? _currentPrice : _confirmPrice
            ...
            if (this.formData.some(f => f.confirmPrice <= 0)) {
                ...
                if (this.$route.query.id) {
                    this.$Modal.confirm({
                        ...
                        loading: true,
                        onOk: () => {
                            confirmForm(this.formData).then(({ data: { code, data, message } }) => {
                                if (code == 200) {
