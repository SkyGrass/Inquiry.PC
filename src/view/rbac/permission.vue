<template>
  <div>
    <Card>
      <tables ref="tables" editable searchable :border="false" size="small" search-place="top" v-model="stores.permission.data" :totalCount="stores.permission.query.totalCount" :columns="stores.permission.columns" @on-delete="handleDelete" @on-edit="handleEdit" @on-select="handleSelect" @on-selection-change="handleSelectionChange" @on-refresh="handleRefresh" :row-class-name="rowClsRender" @on-page-change="handlePageChanged" @on-page-size-change="handlePageSizeChanged">
        <div slot="search">
          <section class="dnc-toolbar-wrap">
            <Row :gutter="16">
              ...
              <Form inline @submit.native.prevent>
                <FormItem>
                  <Input type="text" search :clearable="true" v-model="stores.permission.query.kw" placeholder="输入关键字搜索..." @on-search="handleSearchPermission()">
                  <Select slot="prepend" v-model="stores.permission.query.isDeleted" @on-change="handleSearchPermission" placeholder="删除状态" style="width:60px;">
                    <Option v-for="item in sources.searchSource.isDeletedSources" :value="item.value" :key="item.value">{{item.text}}</Option>
                  </Select>
                  <Select slot="prepend" v-model="stores.permission.query.status" @on-change="handleSearchPermission" placeholder="权限状态" style="width:60px;">
                    <Option v-for="item in sources.searchSource.statusSources" :value="item.value" :key="item.value">{{item.text}}</Option>
                  </Select>
                  <Dropdown slot="prepend" trigger="click" :transfer="true" placement="bottom-start" style="min-width:80px;" @on-visible-change="handleSearchMenuTreeVisibleChange">
                    <Button type="primary">
                    ...
                    <div class="text-left pad10" slot="list" style="min-width:360px;">
                      <div>
                        <Button type="primary" size="small" icon="ios-search" @click="handleRefreshSearchMenuTreeData">刷新菜单</Button>
                        <Button class="ml3" type="primary" size="small" icon="md-close" @click="handleClearSearchMenuTreeSelection">清空</Button>
                      </div>
                      <Tree class="text-left dropdown-tree" :data="sources.searchSource.menuTree.data" @on-select-change="handleSearchMenuTreeSelectChange"></Tree>
                    </div>
                  </Dropdown>
                  </Input>
                </FormItem>
              ...
              <Button icon="md-create" type="primary" @click="handleShowCreateWindow" title="新增权限">新增权限</Button>
              </Col>
            </Row>
          </section>
        </div>
    ...
    <Drawer :title="formTitle" v-model="formModel.opened" width="500" :mask-closable="false" :mask="true" :styles="styles">
      <Form :model="formModel.fields" ref="formPermission" :rules="formModel.rules" label-position="top">
        <Row :gutter="32">
          <Col span="12">
        ...
        <Row>
          ...
          <FormItem label-position="left" prop="menuName">
            ...
            <Dropdown slot="append" trigger="click" :transfer="true" placement="bottom-end">
              ...
              <div class="text-left pad10" slot="list" style="min-width:360px;">
                <div>
                  <Button type="primary" size="small" icon="ios-search" @click="handleRefreshMenuTreeData">刷新菜单</Button>
                </div>
                <Tree class="text-left dropdown-tree" :data="sources.formSource.menuTree.data" @on-select-change="handleMenuTreeSelectChange"></Tree>
              </div>
            </Dropdown>
            </Input>
          </FormItem>
        ...
        <Row :gutter="32">
          ...
          <FormItem label="类型" prop="type">
            <Select v-model="formModel.fields.type" placeholder="权限类型">
              <Option v-for="item in sources.formSource.permissionTypeSources" :value="item.value" :disabled="item.disabled" :key="item.value">{{item.text}}</Option>
            </Select>
          </FormItem>
          </Col>
          <Col span="12">
        ...
        <FormItem label="备注" label-position="top">
          <Input type="textarea" v-model="formModel.fields.description" :rows="4" placeholder="权限备注信息" />
        </FormItem>
      </Form>
