<template>
  <div>
    <Card>
      <tables ref="tables" editable searchable :border="false" size="small" search-place="top" v-model="stores.menu.data" :totalCount="stores.menu.query.totalCount" :columns="stores.menu.columns" @on-delete="handleDelete" @on-edit="handleEdit" @on-select="handleSelect" @on-selection-change="handleSelectionChange" @on-refresh="handleRefresh" :row-class-name="rowClsRender" @on-page-change="handlePageChanged" @on-page-size-change="handlePageSizeChanged">
        <div slot="search">
          <section class="dnc-toolbar-wrap">
            <Row :gutter="16">
              ...
              <Form inline @submit.native.prevent>
                <FormItem>
                  <Input type="text" search :clearable="true" v-model="stores.menu.query.kw" placeholder="输入关键字搜索..." @on-search="handleSearchMenu()">
                  <Select slot="prepend" v-model="stores.menu.query.isDeleted" @on-change="handleSearchMenu" placeholder="删除状态" style="width:60px;">
                    <Option v-for="item in stores.menu.sources.isDeletedSources" :value="item.value" :key="item.value">{{item.text}}</Option>
                  </Select>
                  <Select slot="prepend" v-model="stores.menu.query.status" @on-change="handleSearchMenu" placeholder="菜单状态" style="width:60px;">
                    <Option v-for="item in stores.menu.sources.statusSources" :value="item.value" :key="item.value">{{item.text}}</Option>
                  </Select>
                  <Dropdown slot="prepend" trigger="click" :transfer="true" placement="bottom-start" style="min-width:80px;" @on-visible-change="handleSearchMenuTreeVisibleChange">
                    ...
                    <div class="text-left" slot="list" style="min-width:390px;">
                      <div>
                        <Button type="primary" icon="ios-search" @click="handleRefreshSearchMenuTreeData">刷新菜单</Button>
                        <Button class="ml3" type="primary" icon="md-close" @click="handleClearSearchMenuTreeSelection">清空</Button>
                      </div>
                      <Tree class="text-left dropdown-tree" :data="stores.menu.sources.menuTree.data" @on-select-change="handleSearchMenuTreeSelectChange"></Tree>
                    </div>
                  </Dropdown>
                  </Input>
                </FormItem>
              ...
              <Button icon="md-create" type="primary" @click="handleShowCreateWindow" title="新增菜单">新增菜单</Button>
              </Col>
            </Row>
          </section>
        </div>
    ...
    <Drawer :title="formTitle" v-model="formModel.opened" width="600" :mask-closable="false" :mask="true" :styles="styles">
      <Form :model="formModel.fields" ref="formMenu" :rules="formModel.rules" label-position="left">
        ...
        <Row :gutter="8">
          <Col span="12">
          <FormItem prop="icon">
            <Select v-model="formModel.fields.icon" filterable remote :remote-method="handleLoadIconDataSource" :loading="stores.menu.sources.iconSources.loading" placeholder="请选择图标...">
              <Option v-for="(icon, index) in stores.menu.sources.iconSources.data" :value="icon.code" :key="index">
          ...
          <FormItem label-position="left">
            ...
            <Dropdown slot="append" trigger="click" :transfer="true" placement="bottom-end">
              ...
              <div class="text-left pad10" slot="list" style="min-width:360px;">
                <div>
                  <Button type="primary" icon="ios-search" @click="handleRefreshMenuTreeData">刷新菜单</Button>
                </div>
                <Tree class="text-left dropdown-tree" :data="stores.menuTree.data" @on-select-change="handleMenuTreeSelectChange"></Tree>
              </div>
            </Dropdown>
            </Input>
          </FormItem>
        ...
        <Row>
          ...
          <FormItem label="菜单隐藏" label-position="left">
            <i-switch size="large" v-model="formModel.fields.hideInMenu" :true-value="1" :false-value="0">
              <span slot="open">是</span>
              <span slot="close">否</span>
            </i-switch>
          </FormItem>
        ...
        <FormItem label="备注" label-position="top">
          <Input type="textarea" v-model="formModel.fields.description" :rows="4" placeholder="菜单备注信息" />
        </FormItem>
      </Form>
  ...
  methods: {
    ...
    handleSubmitMenu() {
      this.$refs["formMenu"].validate(valid => {
        ...
        } else { this.$Message.error("请完善表单信息"); }
      });
